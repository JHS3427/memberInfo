...
public class JdbcTemplateProductRepository implements ProductRepository{
    ...
    public JdbcTemplateProductRepository(DataSource dataSource){//dataSource는 application.properties의 DB 정보를 따라 접속함
    ...
    public List<Product> findAll(){
        ...
        //trim(image) 이렇게 쓰면 컬럼명이 trim(image)로 바뀌어서 Product에 없으니 못찾음
        //trim(image) as image로 할 것.
    ...
    public ProductDetailinfo findProductDetailinfo(int pid){
        String sql = "select did, title_en as titleEn, title_ko as titleKo, pid, " +
                " list from product_detailinfo where pid =?";
        ProductDetailinfo productInfo = jdbcTemplate.queryForObject(sql,
    ...
    public List<ProductQna> findQna(int pid){
        String sql = """
                 select  qid, 
                         ...
                         id, 
                         pid, 
                         cdate from product_qna where pid = ?
                 """;
        List<ProductQna> productQna = jdbcTemplate.query(sql,
                                new BeanPropertyRowMapper<>(ProductQna.class),
                                pid);
        return productQna;
    }
    ...
    public ProductReturn findReturn() {
        ...
        return jdbcTemplate.queryForObject(sql,
                new BeanPropertyRowMapper<>(ProductReturn.class));
    }
}
