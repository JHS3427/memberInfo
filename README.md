# bicycle-app

내가 맡은 부분 : 회원정보 관리

내용 : 메인 - 회원가입 / 회원정보 수정(및 조회)/ 로그인 / 아이디,비밀번호 찾기
      다른 분들과 협의 필요 - 자전거 장바구니 / 대여 기록 및 목록(아직 수령 안한건 따로 위로 뺴기)
                            / 나의 고객센터 문의 목록(답변 왔는데 안읽었으면 ! 띄우기) / 방문 여행지 기록

구체적인 내용

메인

로그인 - 아이디, 비밀번호 입력창 + 하단에 아이디/비밀번호찾기,회원가입 넣어두기 + 맨 밑엔 푸터 넣어두기
        - 틀릴경우 빨간색으로 뭐가 틀렸는지 알려주기
        - 아이디는 맞으면 냅두기?
        - 특정아이디 접근시도 횟수제한?

회원정보 조회 및 수정 - 비밀번호 수정/ 핸드폰,이메일 정보 수정/ 주소 수정/ 회원 탈퇴
                    - 보안 설정(카톡으로 보안 확인) / 접속 이력 관리

회원가입 - 페이지 관리자, 판매 및 대여자, 일반사용자로 나누어서 가입받기, 

기본 너비 : 1400px

추가할 수 있는 부분 : 시간이 되면 반응형으로 만들기 - 나의 경우 PC버젼 폰버젼으로 보는게 맞을듯

